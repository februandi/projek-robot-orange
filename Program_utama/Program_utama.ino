...
int speedAwayFromHomeDistance = 500;

int currentGripperAngle = gripperInitialPosition;
...
bool handleGcode() {
  ...
  if (Serial.available()) {
    ...
    if (c == '\r') {
      ...
      if (message.length() > 1000) { // Misal buffer maksimal 100 karakter
...
bool processMessage(const String &msg) {
  ...
  if (command.startsWith("G0")) {
    ...
    // Cek apakah ada perubahan posisi
    bool positionChanged = (targetPosX != stepperX.currentPosition() || 
                            targetPosY != stepperY.currentPosition() ||
                            targetPosZ != stepperZ.currentPosition() || 
                            targetPosE != stepperE.currentPosition());

    if (positionChanged) {
      // Print posisi jika ada perubahan
      ...
      // Run motors until all have reached the target positions
      ...
      // Kirim sendOk jika ada perubahan data dan setelah motor selesai berputar
      sendOk();
    } else {
      // Tidak ada perubahan posisi, tidak mengirim apa-apa
      //Serial.println("No position change, no movement.");
    }
    ...
    if (angle >= gripperInitialPosition && angle <= gripperMaxPosition) {
        if (angle != currentGripperAngle) { // Periksa jika nilai servo berbeda dari sebelumnya
            gripper.write(angle);
            Serial.print("Gripper moved to angle: ");
            Serial.println(angle);
            currentGripperAngle = angle; // Update nilai servo saat ini
            delay(500);
            sendOk(); // Kirim hanya jika nilai berbeda
            return true;
        }
    ...
    // Penanganan perintah K (print positions)
    ...
    Serial.print(stepperX.maxSpeed());  // Assume all steppers have the same speed setting

    Serial.print(" Gripper Angle: ");
    Serial.println(gripper.read());  // Assuming 'gripper' is a Servo object
